    ...
    my $_verifyStartTimeTest = sub {
        eval {
            ...
            $stDt = RMS::Worklogs->_verifyStartTime(
                            $day,
                            ($start) ? DateTime::Format::MySQL->parse_datetime( "$day $start" ) : undef,
...
sub advancedDaily {
    ...
    is($days->{$k[0]}->end()->iso8601(),   '2015-06-11T18:34:30', "1st end");
    is($dtF_hms->format_duration($days->{$k[0]}->duration()), '+08:30:01', "1st duration");
    is($dtF_hms->format_duration($days->{$k[0]}->breaks()),   '+00:00:00', "1st breaks");
    is($dtF_hms->format_duration($days->{$k[0]}->overwork()), '+01:15:01', "1st overwork");

    is($days->{$k[1]}->day(),              '2015-11-03',          "2nd day");
    is($days->{$k[1]}->start()->iso8601(), '2015-11-03T08:24:38', "2nd start");
    is($days->{$k[1]}->end()->iso8601(),   '2015-11-03T18:48:14', "2nd end");
    is($dtF_hms->format_duration($days->{$k[1]}->duration()), '+10:15:00', "2nd duration");
    is($dtF_hms->format_duration($days->{$k[1]}->breaks()),   '+00:08:36', "2nd breaks");
    is($dtF_hms->format_duration($days->{$k[1]}->overwork()), '+03:00:00', "2nd overwork");

    is($days->{$k[2]}->day(),              '2015-11-04',          "3rd day");
    is($days->{$k[2]}->start()->iso8601(), '2015-11-04T11:37:06', "3rd start");
    is($days->{$k[2]}->end()->iso8601(),   '2015-11-04T19:36:48', "3rd end");
    is($dtF_hms->format_duration($days->{$k[2]}->duration()), '+07:59:42', "3rd duration");
    is($dtF_hms->format_duration($days->{$k[2]}->breaks()),   '+00:00:00', "3rd breaks");
